...
//    lateinit var storedVerificationIdlogin: String
...
//        dialog = AlertDialog.Builder(this).setView(R.layout.activity_login).create()
        ...
        //check user jika tidak kosong maka ke profile
        if (currentUser != null) {
...
//            Toast.makeText(applicationContext, "user $phone", Toast.LENGTH_LONG).show()
        ...
        login.setOnClickListener {
            ...
            if(hp.isNotEmpty() && password.isNotEmpty()) {
                hp="+62"+hp
                checkPhonePassExist(hp, password)
            }
            ...
            if (hp.isEmpty() && password.isNotEmpty()) {
                Toast.makeText(this@LoginActivity, "Nomor Handphone kosong ! ", Toast.LENGTH_SHORT).show()
            }
            if (password.isEmpty() && hp.isEmpty()) {
                Toast.makeText(this@LoginActivity, "No Hp dan Password Kosong ! ", Toast.LENGTH_SHORT).show()
            }
        }
    }
